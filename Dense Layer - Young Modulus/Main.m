...
xSteps              = 20;                    
ySteps              = 20;
%Diameter SampleCell /Size SampleCell in µm:
SizeCell            = 20;   
...
for i = 1:size(Reconstructed,1)
    ...
    if abs(diff(FitRange))>2 && FitRange(1)<FitRange(2)
        [tmpTension,tmpRSquaredTension] = ...
            SurfaceTension(CroppedDataX(:,i),tmpYCorrected,ContactPoint,SpringConstant,SizeCell,DiameterIndenter,FitRange);
...
%smooth with averaging of 2 basically works like a linear interpolation of NaN (but just in 1d ...):
YoungMap = smooth(YoungMap,2);
IndendationMap(KeptCurves) = IndendationMaxFitForce;  
IndendationMap = smooth(IndendationMap,2);
